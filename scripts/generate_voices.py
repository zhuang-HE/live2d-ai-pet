"""
Hermes Pet - 内置语音生成脚本 (本地版)
使用 Windows SAPI 引擎 (Microsoft Huihui 女声) 生成 WAV 文件
...
def main():
    ...
    print(f"\n🎙️  使用语音: Microsoft Huihui (慧慧 - 中文女声)")
    print(f"📁 输出目录: {OUTPUT_DIR}")
    print(f"📝 共 {len(PHRASES)} 句台词\n")
    
    engine = pyttsx3.init()
    engine.setProperty('voice', 'HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Speech\\Voices\\Tokens\\TTS_MS_ZH-CN_HUIHUI_11.0')
    engine.setProperty('rate', 180)   # 语速 (默认 200, 稍慢一点更可爱)
    engine.setProperty('volume', 0.9)
    
    for filename, text in PHRASES:
        output_path = os.path.join(OUTPUT_DIR, f"{filename}.wav")
        engine.save_to_file(text, output_path)
        print(f"  🔄 {filename}.wav - {text[:25]}...")
    ...
    print(f"\n✨ 完成! {len(PHRASES)} 个文件, 总大小 {total/1024:.1f} KB")
